...
import { SpinIcon, PlayIcon, SearchIcon, SteamIcon, EpicIcon, GogIcon, CustomGameIcon, HeartIcon, LibraryIcon, SparkleIcon } from "@/components/ui/Icons";
...
function Wheel({ pool, rotation, isSpinning }: { pool: Game[]; rotation: ReturnType<typeof useMotionValue<number>>; isSpinning: boolean }) {
  const n = pool.length;
  const segAngle = n > 0 ? 360 / n : 0;
  const fontSize = n <= 6 ? 11 : n <= 12 ? 9 : 7.5;
  const maxChars = n <= 6 ? 16 : n <= 12 ? 12 : 8;
  const textR = R * 0.62;
  ...
  return (
    <div className="relative select-none" style={{ width: SIZE, height: SIZE }}>
      <div
        ...
        style={{
          background: isSpinning
            ? "radial-gradient(circle, rgb(var(--accent-500) /0.25) 0%, rgb(var(--accent-600) /0.08) 50%, transparent 70%)"
          ...
          return (
            <g key={game.id}>
              ...
              <text
                ...
                fill="white"
                opacity="0.95"
                style={{ textShadow: "0 1px 4px rgba(0,0,0,0.9)", fontFamily: "inherit", pointerEvents: "none" }}
      ...
      >
        ...
        <circle cx={CX} cy={CY} r={R + 10} fill="none" stroke="rgb(var(--accent-500) /0.08)" strokeWidth="1" />

        {Array.from({ length: 60 }).map((_, i) => {
          const a = (i / 60) * 360;
          const isMajor = i % 5 === 0;
          const inner = polar(a, R + 5);
          const outer = polar(a, R + (isMajor ? 15 : 10));
          return (
            <line
              key={i}
              x1={inner.x} y1={inner.y}
              x2={outer.x} y2={outer.y}
              stroke={isMajor ? "rgb(var(--accent-500) /0.4)" : "rgb(var(--accent-500) /0.15)"}
        ...
        <polygon
          points={`${CX},${CY - R - 6} ${CX - 12},${CY - R + 18} ${CX + 12},${CY - R + 18}`}
          fill="url(#pointerGrad)"
          filter="drop-shadow(0 3px 8px rgb(var(--accent-600) /0.8))"
        />
        <polygon
          points={`${CX},${CY - R - 6} ${CX - 12},${CY - R + 18} ${CX + 12},${CY - R + 18}`}
          fill="none"
        ...
        <circle cx={CX} cy={CY} r={INNER_R + 2} fill="none" stroke="rgb(var(--accent-500) /0.3)" strokeWidth="1.5" />
        <circle cx={CX} cy={CY} r={INNER_R} fill="#0a0914" />
        <circle cx={CX} cy={CY} r={INNER_R - 3} fill="none" stroke="rgb(var(--accent-500) /0.2)" strokeWidth="1" />

        <text
          x={CX} y={CY - 5}
          textAnchor="middle" dominantBaseline="middle"
          fontSize="10" fontWeight="800" fill="rgb(var(--accent-500) /0.7)"
        ...
        <text
          x={CX} y={CY + 8}
          textAnchor="middle" dominantBaseline="middle"
          fontSize="8" fontWeight="600" fill="rgb(var(--accent-500) /0.4)"
...
function WinnerCard({ game, onPlayAgain }: { game: Game; onPlayAgain: () => void }) {
  ...
  return (
    <motion.div
      ...
      style={{
        ...
        boxShadow: "0 0 60px rgb(var(--accent-600) /0.15), 0 20px 40px rgba(0,0,0,0.3)",
      }}
    >
      <motion.div
      ...
      <div className="relative flex items-center gap-5 p-5">
        <div className="w-20 h-[100px] rounded-xl overflow-hidden shrink-0 border border-white/10"
        ...
        <div className="flex-1 min-w-0">
          ...
          <p className="text-[18px] font-bold text-white leading-tight truncate mb-2">{game.name}</p>
...
export default function Spin() {
  ...
  const pool = useMemo(
    () => allGames.filter((g) => selected.has(g.id)).slice(0, 20),
    [allGames, selected]
  );
        ...
        >
          ...
          <div className="px-4 py-2.5 shrink-0" style={{ borderTop: "1px solid rgba(255,255,255,0.04)" }}>
            <div className="flex items-center justify-between">
              ...
              {selected.size > 20 && (
                <p className="text-[10px] text-amber-500 font-medium">wheel shows 20</p>
              )}
            </div>
          </div>
        </div>

        <div className="flex-1 flex flex-col items-center justify-center gap-8 p-6 overflow-auto relative">
